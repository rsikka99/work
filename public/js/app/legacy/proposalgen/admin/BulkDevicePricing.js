require(['jquery', 'jqgrid'], function ($)
{
    ...
    $document.ready(function ()
    {
        ...
        var $toners_list = $("#toners_list");
        var $toners_table = $("#toners_table");
        var $pricing_filter = $("#pricing_filter");
        var $criteria_filter = $("#criteria_filter");
        var $text_criteria = $("#text_criteria");
        var $txt_criteria = $("#txtCriteria");
        var $list_criteria = $("#list_criteria");
        var $cbo_criteria = $("#cboCriteria");
        var $default_price = $("#default_price");
        var $details_error = $('#details_error');
        var $hdn_page = $("#hdnPage");
        var $button_update = $("#btnUpdate");
        var $button_apply = $("#btnApply");
        var $text_update = $("#txtUpdate");
        ...
        var setupManufacturerList = function (url)
        {
            $.ajax({
                ...
                success : function (data)
                {
                    ...
                    {
                        $cbo_criteria.append($("<option></option>").val(data.rows[i].cell[0]).text(data.rows[i].cell[1]));
                    }
                },
                error   : function ()
                {
            ...
            if (percentage > 0)
            {
                ...
                for (var i = 0; i < ids.length; i++)
                {
                    ...
                    if ($pricing_filter.val() == 'printer')
                    {
                         ...
                         */
                        var laborCostPerPageElement = $("#laborCostPerPage" + jqGridRow.masterID);
                        var partsCostPerPageElement = $("#partsCostPerPage" + jqGridRow.masterID);
        ...
        $devices_list.jqGrid({
            ...
            gridComplete: function ()
            {
                ...
                for (var i = 0; i < ids.length; i++)
                {
                    ...
                    row.new_labor_cost_per_page = "$<input type='text' id='laborCostPerPage" + row.masterID + "' name='laborCostPerPage" + row.masterID + "' class='form-group input-small text-right int-only' maxlength='12' />";
                    row.new_parts_cost_per_page = "$<input type='text' id='partsCostPerPage" + row.masterID + "' name='partsCostPerPage" + row.masterID + "' class='form-group input-small text-right int-only' maxlength='12' />";
        ...
        $toners_list.jqGrid({
            ...
            colModel    : [
                ...
                },
                {
                    ...
                    formatter    : 'currency',
                    align        : 'right',
                    formatoptions: {decimalPlaces: 2, defaultValue: '-'},
                    sorttype     : 'int'
                },
                {
                    width        : 60,
            ...
            gridComplete: function ()
            {
                ...
                for (var i = 0; i < ids.length; i++)
                {
                    ...
                    row.new_toner_price = "$ <input type='text' name='txtTonerPrice" + row.toner_id + "' id='txtTonerPrice" + row.toner_id + "' class='form-control input-mini int-only' maxlength='8' />";
                    row.new_dealer_sku = "<input type='text' name='txtNewDealerSku" + row.toner_id + "' id='txtNewDealerSku" + row.toner_id + "' class='form-control input-small' maxlength='255' />";
                    ...
                    {
                        var $inner = $('<div class="inner"></div>').hide();
