...
class Quotegen_PageController extends Zend_Controller_Action
{
    ...
    public function indexAction ()
    {
        ...
        $pageMapper = Quotegen_Model_Mapper_QuoteDevicePage::getInstance();
        $paginator = new Zend_Paginator(new My_Paginator_MapperAdapter($pageMapper));
    ...
    public function deleteAction ()
    {
        ...
        {
            $this->_helper->flashMessenger(array (
                    'danger' => 'There was an error finding that page to delete.' 
            ));
            $this->_helper->redirector('index');
        }
        
        $message = "Are you sure you want to delete {$page->getName()}?";
        ...
        if ($request->isPost())
        {
            ...
            if (! isset($values ['cancel']))
            {
                ...
                {
                    Quotegen_Model_Mapper_OptionCategory::getInstance()->deleteByOptionId($page->getId());
                    $pageMapper->delete($page);
                    
                    $this->_helper->flashMessenger(array (
                            'success' => "Option  {$this->view->escape ( $page->getName() )} was deleted successfully." 
                    ));
                    $this->_helper->redirector('index');
                }
            }
