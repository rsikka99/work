...
class Healthcheck_ViewModel_Healthcheck extends Healthcheck_ViewModel_Abstract
{
    ...
    public function __construct (Healthcheck_Model_Healthcheck $report)
    {
        ...
        $assessmentSettings = $this->healthcheck->getHealthcheckSettings();
        ...
        Proposalgen_Model_MasterDevice::setReportMargin(1 - ((((int)$assessmentSettings->assessmentReportMargin)) / 100));
...
//            $uniqueDevicesGraph->setLabels($labels);
    ...
    public function calculateDealerMonthlyRevenueUsingTargetCostPerPage ()
    {
        if (!isset($this->_dealerMonthlyRevenueUsingTargetCostPerPage))
        {
            ...
            foreach ($this->getPurchasedDevices() as $deviceInstance)
            {
                $this->_dealerMonthlyRevenueUsingTargetCostPerPage += $deviceInstance->getAverageMonthlyBlackAndWhitePageCount() * $this->assessment->getAssessmentSettings()->targetMonochromeCostPerPage;
                $this->_dealerMonthlyRevenueUsingTargetCostPerPage += $deviceInstance->getAverageMonthlyColorPageCount() * $this->assessment->getAssessmentSettings()->targetColorCostPerPage;
            }
        }
