...
class Healthcheck_ViewModel_Ranking
{
     ...
     *
     * @param \Healthcheck_ViewModel_Healthcheck $proposal
     */
    public function __construct (Healthcheck_ViewModel_Healthcheck $proposal)
    {
    ...
    public function getSuppliesAndServiceLogistics ()
    {
        if (!isset($this->SuppliesAndServiceLogistics))
        {
            ...
            {
                $rankingText .= " To improve your score in this area, " . $this->proposal->healthcheck->getClient()->companyName . " could ";
    ...
    public function getPrintingHardwareUsage ()
    {
        if (!isset($this->PrintingHardwareUsage))
        {
            ...
            $AverageMonthlyPrintVolumePerEmployee = Functions::getValueFromRangeStepTable($this->proposal->getDevices()->allIncludedDeviceInstances->getPageCounts()->getCombinedPageCount()->getMonthly() / $this->proposal->getEmployeeCount(), $criteria ["AverageMonthlyPrintVolumePerEmployee"]);
            $NumberOfEmployeesPerDevice           = Functions::getValueFromRangeStepTable($this->proposal->getEmployeeCount() / $this->proposal->getDevices()->allIncludedDeviceInstances->getCount(), $criteria ["NumberOfEmployeesPerDevice"]);
    ...
    public function getTechnologyReliabilityAndUserProductivity ()
    {
        if (!isset($this->TechnologyReliabilityAndUserProductivity))
        {
            ...
            $ScanCapable        = Functions::getValueFromRangeStepTable(($this->proposal->getDevices()->scanCapableDeviceInstances->getCount() / $this->proposal->getDevices()->allIncludedDeviceInstances->getCount()) * 100, $criteria ["ScanCapable"]);
            $FaxCapable         = Functions::getValueFromRangeStepTable(($this->proposal->getDevices()->faxCapableDeviceInstances->getCount() / $this->proposal->getDevices()->allIncludedDeviceInstances->getCount()) * 100, $criteria ["FaxCapable"]);
            $ColorCapable       = Functions::getValueFromRangeStepTable(($this->proposal->getDevices()->colorCapableDeviceInstances->getCount() / $this->proposal->getDevices()->allIncludedDeviceInstances->getCount()) * 100, $criteria ["ColorCapable"]);
    ...
    public function getEnvironmentalFriendliness ()
    {
        if (!isset($this->EnvironmentalFriendliness))
        {
            ...
            $DuplexCapable = Functions::getValueFromRangeStepTable(($this->proposal->getDevices()->duplexCapableDeviceInstances->getCount() / $this->proposal->getDevices()->allIncludedDeviceInstances->getCount()) * 100, $criteria ["DuplexCapable"]);
            $ScanCapable   = Functions::getValueFromRangeStepTable(($this->proposal->getDevices()->scanCapableDeviceInstances->getCount() / $this->proposal->getDevices()->allIncludedDeviceInstances->getCount()) * 100, $criteria ["ScanCapable"]);
