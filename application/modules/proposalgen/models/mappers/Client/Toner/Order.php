 ...
 */
class Proposalgen_Model_Mapper_Client_Toner_Order extends My_Model_Mapper_Abstract
{
    /*
     * Column Definitions
     */
     ...
     *
     */
    protected $_defaultDbTable = 'Proposalgen_Model_DbTable_Client_Toner_Order';

    /**
     * Gets an instance of the mapper
     *
     * @return Proposalgen_Model_Mapper_Client_Toner_Order
     */
     ...
     *
     * @param $object Proposalgen_Model_Client_Toner_Order
    ...
    {
        if ($object instanceof Proposalgen_Model_Client_Toner_Order)
        {
            $whereClause = array(
     ...
     *
     * @return Proposalgen_Model_Client_Toner_Order
     */
    public function find ($id)
    {
        // Get the item from the cache and return it if we find it.
        $result = $this->getItemFromCache($id);
        if ($result instanceof Proposalgen_Model_Client_Toner_Order)
        {
            return $result;
        }
        ...
        $row    = $result->current();
        $object = new Proposalgen_Model_Client_Toner_Order($row->toArray());
    ...
    /**
     * @param Proposalgen_Model_Client_Toner_Order $object
     *
     * @return int
     */
...
FROM client_toner_orders
    ...
    LEFT JOIN dealer_toner_attributes
        ON dealer_toner_attributes.tonerId = client_toner_orders.tonerId
           AND dealer_toner_attributes.dealerId = ?
...
FROM client_toner_orders
    ...
    LEFT JOIN dealer_toner_attributes AS replacement_dta
        ...
        $whereClause = "WHERE client_toner_orders.clientId = ?";
        $bindData[]  = $clientId;
        if ($filter == 'sku' && $criteria)
        {
            $whereClause .= " AND (toners.sku LIKE ? OR dealer_toner_attributes.dealerSku LIKE ? OR client_toner_orders.clientSku LIKE ?)";
            $bindData[] = "%$criteria%";
            $bindData[] = "%$criteria%";
            $bindData[] = "%$criteria%";
        }
        ...
        {
            $whereClause .= " AND client_toner_orders.cost LIKE ? ";
            $bindData[] = "%$criteria%";
        }
     ...
     * @param bool $justCount
     *
     * @return Proposalgen_Model_Client_Toner_Order[]|int
     */
    public function fetchAllForClient ($clientId, $dealerId, $order = null, $count = 25, $offset = 0, $filter = null, $criteria = null, $justCount = false)
    {
        ...
        foreach ($clientTonerOrderArray as $clientTonerOrderData)
        {
            ...
            if (!$object instanceof Proposalgen_Model_Client_Toner_Order)
            {
                $object = new Proposalgen_Model_Client_Toner_Order($clientTonerOrderData);
