...
class Proposalgen_GrossmarginController extends My_Controller_Report
{
    ...
    {
        $this->_redirect("/proposal/grossmargin/generate");
    }
    ...
    public function initCSVGrossMargin ()
    {
        ...
        try
        {
            ...
            $fieldTotals = array ();
            $fieldTotals [] = 'Totals for ' . $proposal->DeviceCount . ' devices:';
            $fieldTotals [] = number_format($proposal->PageCounts->Purchased->BlackAndWhite->Monthly, 0, '.', '');
            $fieldTotals [] = '';
            $fieldTotals [] = '';
            $fieldTotals [] = '';
            $fieldTotals [] = '$' . number_format($proposal->GrossMarginTotalMonthlyCost->BlackAndWhite, 2, '.', '');
            $fieldTotals [] = number_format($proposal->PageCounts->Purchased->Color->Monthly, 0, '.', '');
            $fieldTotals [] = '';
            $fieldTotals [] = '';
            $fieldTotals [] = '';
            $fieldTotals [] = '$' . number_format($proposal->GrossMarginTotalMonthlyCost->Color, 2, '.', '');
        }
        catch ( Exception $e )
        {
