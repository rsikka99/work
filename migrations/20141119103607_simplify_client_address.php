...
class SimplifyClientAddress extends AbstractMigration
{
    /**
     * Migrate Up.
     */
    public function up ()
    {
        $addressTable = $this->table('addresses');
        $addressTable
            ->dropForeignKey('countryId')
            ->update();

        $this->dropTable('regions');
        $this->dropTable('countries');

        $this->execute('UPDATE addresses SET countryId = 39  WHERE countryId = 1');
        $this->execute('UPDATE addresses SET countryId = 227 WHERE countryId = 2');

        $countriesTable = $this->table('countries', array('id' => false, 'primary_key' => 'country_id'));
        $countriesTable
            ->addColumn('country_id', 'integer')
            ->addColumn('name', 'string')
            ->addColumn('iso_alpha2', 'string')
            ->addColumn('iso_alpha3', 'string')
        ...
        $this->execute('
        INSERT INTO `countries` (`country_id`, `name`, `iso_alpha2`, `iso_alpha3`, `iso_numeric`, `currency_code`, `currency_name`, `currency_symbol`, `flag`)
...
');
        ...
        $addressTable
            ->addForeignKey('countryId', 'countries', 'country_id')
            ->save();
    ...
    public function down ()
    {
        ...
        $this->execute('UPDATE addresses SET countryId = 1 WHERE countryId = 39 ');
        $this->execute('UPDATE addresses SET countryId = 2 WHERE countryId = 227');
    ...
    PRIMARY KEY (`id`)
        ...
        $this->execute('DELETE FROM countries WHERE TRUE');
        $this->execute('INSERT INTO `countries` (`id`, `name`, `locale`) VALUES
(1, \'Canada\', \'en_CA\'),
(2, \'United States\', \'en_US\');');

        $addressTable
            ->addForeignKey('countryId', 'countries', 'id')
            ->save();
